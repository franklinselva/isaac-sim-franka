...
def robot_state_publisher_spawner(context: LaunchContext, load_gripper, ee_id):
    load_gripper_str = context.perform_substitution(load_gripper)
    ee_id_str = context.perform_substitution(ee_id)
    franka_xacro_filepath = os.path.join(
        get_package_share_directory("franka_demo"),
        "urdf/robot/",
        "panda/panda.urdf.xacro",
    )
...
def generate_launch_description():
    ...
    robot_state_publisher_spawner_opaque_function = OpaqueFunction(
        function=robot_state_publisher_spawner, args=[load_gripper, ee_id]
    )
    ...
    return LaunchDescription(
        [
            ...
            Node(
                ...
                remappings=[("/joint_states", "/isaac_joint_commands")],
            ),
            Node(
                package="rviz2",
                executable="rviz2",
